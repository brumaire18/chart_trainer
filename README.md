...
日米業種リードラグ画面には **「データ健全性チェック」** を追加しており、US/JP の各銘柄ごとに次を一覧表示します。

- `symbol`
- `latest_date`
- `stale_days`（最新営業日との差分営業日数）
- `missing_recent_dates`（直近 N 営業日の欠落日）
- `missing_value_rows`（OHLCV 欠損行数）
- `duplicate_dates_count`
- `is_usable`

`stale_days` / `missing_recent_dates` は、単純な日次連番ではなく「市場別（`market`）の営業日」を前提に判定します。現状の既定は **平日ベース** ですが、実装は `market` 分岐を持っているため、将来的に US/JP の祝日カレンダーへ拡張可能です。

> 重要: 現状は平日ベース判定のため、US と JP の祝日ズレ（片方だけ休場）でも「欠落候補」として検知されることがあります。  
> そのため `is_usable=False` は「即データ破損」とは限らず、まずは警告として原因（祝日差か、実欠落か）を確認してください。

バックテスト実行前に「欠損銘柄の扱い」を選択できます。

- **欠損銘柄を除外して続行**: `is_usable=False` を除外して実行
- **警告のみ**: 警告表示のみで全銘柄を対象に実行
- **欠損があれば停止**: 1 銘柄でも `is_usable=False` があれば実行停止

画面上で CSV 保存 → 健全性確認 → ポリシー選択 → バックテスト実行、まで完結します。
